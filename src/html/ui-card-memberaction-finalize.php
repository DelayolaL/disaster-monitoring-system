...
if ($_SERVER['REQUEST_METHOD'] == 'POST'){
  ...
  $sql_query = "UPDATE `family_members` SET `Member_Name` = COALESCE(NULLIF('$upMem',''), `Member_Name`), `Head_Relation` = COALESCE(NULLIF('$upRel',''), `Head_Relation`), `Age` = COALESCE(NULLIF('$upAge',''), `Age`), `Gender` = COALESCE(NULLIF('$upGender',''), `Gender`), `Civil_Status` = COALESCE(NULLIF('$upCiv',''), `Civil_Status`), `Educational_Level` = COALESCE(NULLIF('$upEduc',''), `Educational_Level`), `Occupational_Skills` = COALESCE(NULLIF('$upOcc',''), `Occupational_Skills`), `Remarks` = COALESCE(NULLIF('$upRem',''), `Remarks`), `Casualty` = COALESCE(NULLIF('$upCas',''), `Casualty`) WHERE `family_members`.`member_ID` = '$theID'";


$sql = "SELECT * FROM `family_members` WHERE `member_ID` = '$theID';";
...
if (mysqli_query($conn, $sql_query)) 
{
  ...
  $recentact = "Family Member [".$MemberName."] Has Been Updated";
...
<div class="container-xl">
      <div class="table-title">
        <div class="row">
          <div class="col-sm-12">
            <center>
            <h2 style="color: white;">Family Member<b>Updated!</b></h2>
            </center>
          </div>
        </div>
      </div>
...
<?php
  ...
  $sql_query = "DELETE FROM family_members WHERE `family_members`.`member_ID` = '$theID';";
...
$recentact = "Family Member [".$MemberName."] Has Been Deleted";
...
<div class="container-xl">
      <div class="table-title">
        <div class="row">
          <div class="col-sm-12">
            <center>
            <h2 style="color: white;">Family Member <b>Deleted!</b></h2>
            </center>
          </div>
        </div>
      </div>
