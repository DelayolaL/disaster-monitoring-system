...
if (isset($_POST['Save'])) {
  ...
  $evaccent = $_POST['evaccenter'];
  $assisduring = $_POST['asstduring'];

  $sql = "SELECT * FROM `disaster` WHERE `Disaster_ID` = '$assisduring';";
  ...
  // Member values
  if (isset($_POST['casualty'])) {
    $casualty = $_POST['casualty'];
  }
  if (isset($_POST['memevaccenter'])) {
    $memevaccenter = $_POST['memevaccenter'];
  }

  $evacsql = "UPDATE `family` SET `Evac_Cent_ID` = '$evaccent' WHERE `family`.`Serial_No.` = '$table';";
  // echo "$evacsql <br><br>";
  mysqli_query($conn, $evacsql);
   $damagesql = "INSERT INTO `damages` (`damage_ID`, `Serial_No.`, `Disaster_ID`, `Evac_Cent_ID`, `Date`, `Assistance During:`, `Housing_Condition`, `Provided`) VALUES (NULL, '$table', $assisduring, $evaccent, curdate(), '$disastername', '$damages', 'No');";

   $sql = "SELECT * FROM `family_members` WHERE `Serial_No.` = '$table';";
   $result = $conn->query($sql);
  ...
  while ($row = $result->fetch_assoc()) {
    $indexofid = $row['member_ID'];
    $sqltest = "UPDATE `family_members` SET `Casualty` = '".$casualty[$cellindex]."' WHERE `family_members`.`member_ID` = $indexofid;";

    $sqltest2 = "UPDATE `family_members` SET `Evac_Cent_ID` = '".$memevaccenter[$cellindex]."' WHERE `family_members`.`member_ID` = $indexofid;";

    if ($casualty[$cellindex] != '') {
      // echo "$sqltest <br>";
      mysqli_query($conn, $sqltest);
    }
    // echo "$sqltest2 <br><br>";
    mysqli_query($conn, $sqltest2);
    $cellindex = $cellindex + 1;
  }
  // echo "$damagesql <br><br>";
  mysqli_query($conn, $damagesql);
  $finalsql = "UPDATE `family` SET `Damage_Rec` = 'Yes' WHERE `family`.`Serial_No.` = '$table';";
  // echo "$finalsql <br><br>";
  mysqli_query($conn, $finalsql);

  $famsql = "SELECT * FROM `family` WHERE `Serial_No.` = '$table';";
  $result = $conn->query($famsql);
  while ($row = $result->fetch_assoc()) {
    $familyname = $row['Family_Name'];
  }

  $recentact = "Added Damage Record for ".$familyname." Family as ".$damages." during ". $disastername;
  $recentsql = "INSERT INTO `recent activity` (`Act_ID`, `Activity`, `Account_Used`) VALUES (NULL, '$recentact', '$profile');";
  // echo "$recentsql <br><br>";
  ...
  <center>
  <div class="buttons">
    <button type="button" class="btn btn-default btn-lg" onclick="window.location.replace('assistance.php')">Return</button>
  </div>
  </center>
</body>
</html>
