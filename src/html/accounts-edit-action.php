...
if ($_SERVER['REQUEST_METHOD'] == 'POST'){
  ...
  $priviledge = $_POST['updatePriv'];

  

  $sql = "SELECT * FROM `accounts` WHERE `Account_ID` = '$theID';";
  ...
  $sql_query = "UPDATE `accounts` SET `Username` = COALESCE(NULLIF('$username',''), `Username`), `Email` = COALESCE(NULLIF('$email',''), `Email`), `Password` = COALESCE(NULLIF('$password',''), `Password`), `Priviledge` = COALESCE(NULLIF('$priviledge',''), `Priviledge`) WHERE `accounts`.`Account_ID` = '$theID';";
if (mysqli_query($conn, $sql_query)) 
{

if ($username == '') {
  $recentact = "Account [".$TheUsername."] Has Been Updated";
}
elseif ($username != '') {
  $recentact = "Account [".$TheUsername."] Username has been Updated to [".$username."]";
}
else{
  $recentact = "An account Has Been Updated";
}
...
<div class="container-xl">
      <div class="table-title">
        <div class="row">
          <div class="col-sm-12">
            <center>
            <h2 style="color: white;">Account <b>Updated!</b></h2>
            </center>
          </div>
        </div>
      </div>
      <center>
      <button type="button" onclick="location.replace('accounts.php')" class="btn btn-default btn-lg">Return</button>
      </center>    
</div>
<?php
  }
  ...
  $sql_query = "DELETE FROM accounts WHERE `accounts`.`Account_ID` = '$theID'";
if (mysqli_query($conn, $sql_query)) 
{

$recentact = "Account [".$TheUsername."] Has Been Deleted";
$recentsql = "INSERT INTO `recent activity` (`Act_ID`, `Activity`, `Activity_Type`, `Account_Used`) VALUES (NULL, '$recentact', 'DELETE', '$profile');";
mysqli_query($conn, $recentsql);
...
<div class="container-xl">
      <div class="table-title">
        <div class="row">
          <div class="col-sm-12">
            <center>
            <h2 style="color: white;">Account <b>Deleted!</b></h2>
            </center>
          </div>
        </div>
      </div>
