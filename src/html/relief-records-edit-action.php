...
if ($_SERVER['REQUEST_METHOD'] == 'POST'){
  ...
  $theID = $_POST['Update'];
  $upKind = $_POST['upKind'];
  $upQuant = $_POST['upQuant'];
  $upRecieve = $_POST['upRecieve'];
  $upCost = $_POST['upCost'];
  $upProvider = $_POST['upProvider'];

  $sql_query = "UPDATE `relief_distribution` SET `Rec_Fam_Member` = COALESCE(NULLIF('$upRecieve',''), `Rec_Fam_Member`), `Kind/Type` = COALESCE(NULLIF('$upKind',''), `Kind/Type`), `Quantity` = COALESCE(NULLIF('$upQuant',''), `Quantity`), `Cost` = COALESCE(NULLIF('$upCost',''), `Cost`), `Provider` = COALESCE(NULLIF('$upProvider',''), `Provider`) WHERE `relief_distribution`.`rd_ID` = '$theID';";
      ...
      <div class="table-title">
        <div class="row">
          <div class="col-sm-12">
            ...
            <p><?php echo $sql_query; ?></p>
            </center>
          </div>
        </div>
      </div>
      <center>
      <button type="button" onclick="location.replace('relief-records.php')" class="btn btn-default btn-lg">Return</button>
      </center>    
</div>
<?php
  }
  ...
  elseif (isset($_POST['Delete'])) {
    ...
    $sql_query = "DELETE FROM `relief_distribution` WHERE `relief_distribution`.`rd_ID` = '$theID';";
    if (mysqli_query($conn, $sql_query)) {
?>
<div class="container-xl">
      <div class="table-title">
